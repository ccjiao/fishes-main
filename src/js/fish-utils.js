...
let BACKEND_URL;
const runtimeBackend = window.ENV && window.ENV.BACKEND_URL ? window.ENV.BACKEND_URL : null;
if (forceLocal) {
    BACKEND_URL = runtimeBackend || 'http://localhost:8080';
} else if (forceProd) {
    BACKEND_URL = runtimeBackend || 'https://fishes-be-571679687712.northamerica-northeast1.run.app';
} else {
    BACKEND_URL = runtimeBackend || (isLocalhost ? 'http://localhost:8080' : 'https://fishes-be-571679687712.northamerica-northeast1.run.app');
}
